...
class CrawlerDetector:
    ...
    def detect(self, frame, is_bgr=False, do_display_detection=False):
        ...
        hm, elapsed_time = self._detect_crawler(frame_tensor)
        # bb = self._restore_bb_size(bb, frame.shape)
        # display detection
        if do_display_detection:
            self._display_hm(crop_frame, hm, elapsed_time)
        return hm
    ...
    def _preprocess_frame(self, frame, is_bgr):
        ...
        # crop_frame = cv2.resize(crop_frame, (self._opt.net_image_size/2, self._opt.net_image_size/2))
    ...
    def _detect_crawler(self, frame_tensor):
        ...
        hm = self._model.test(frame_tensor, do_normalize_output=False)
        elapsed_time = (datetime.datetime.now() - start_time).total_seconds()
        return hm[0], elapsed_time
    ...
    def _display_hm(self, frame, hm, elapsed_time):
        # display hm
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if ret:
            hm = crawler_detector.detect(frame, is_bgr=True, do_display_detection=True)
